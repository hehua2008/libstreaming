 ...
 * class directly.
 * Call {@link #setDestinationAddress(InetAddress)}, {@link #setDestinationPorts(int)}
 * and {@link #setAudioQuality(AudioQuality)}
 * to configure the stream. You can then call {@link #start()} to start the RTP stream.
 * Call {@link #stop()} to stop the stream.
 */
public class AACStream extends AudioStream {
    public static final String TAG = AACStream.class.getSimpleName();
    ...
    /**
     * Some data (the actual sampling rate used by the phone and the AAC profile) needs to be stored
     * once {@link #getSessionDescription()} is called.
     *
    ...
    public synchronized void configure() throws IllegalStateException, IOException {
        ...
        if (mMode == MODE_MEDIARECORDER_API) {
            ...
            mProfile = 2; // AAC LC
            mChannel = 1;
            mConfig = (mProfile & 0x1F) << 11 | (mSamplingRateIndex & 0x0F) << 7
                    | (mChannel & 0x0F) << 3;
    ...
    private void testADTS() throws IllegalStateException, IOException {
        ...
        // The structure of an ADTS packet is described here: http://wiki.multimedia.cx/index
        // .php?title=ADTS
        ...
        // 5 bits for the object type / 4 bits for the sampling rate / 4 bits for the channel /
        // padding
        mConfig =
                (mProfile & 0x1F) << 11 | (mSamplingRateIndex & 0x0F) << 7 | (mChannel & 0x0F) << 3;
