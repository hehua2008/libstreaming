...
public class RtspClient {
     ...
     * You need to call {@link #setServerAddress(String, int)} and optionally {@link
     * #setSession(Session)}
        ...
        if (response.status == 401) {
            ...
            if (mParameters.username == null || mParameters.password == null) {
                throw new IllegalStateException(
                        "Authentication is enabled and setCredentials(String,String) was not "
                                + "called !");
            }
            try {
    ...
    private void sendRequestSetup() throws IllegalStateException, SocketException, IOException {
        for (int i = 0; i < 2; i++) {
            ...
            if (stream != null) {
                String params =
                        mParameters.transport == TRANSPORT_TCP ? ("TCP;interleaved=" + 2 * i + "-"
                                + (2 * i + 1))
                                : ("UDP;unicast;client_port=" + (5000 + 2 * i) + "-" + (5000 + 2 * i
                                        + 1) + ";mode=receive");
                ...
                if (mParameters.transport == TRANSPORT_UDP) {
                    try {
                        ...
                        Log.d(TAG,
                                "Setting destination ports: " + Integer.parseInt(m.group(3)) + ", "
                                        + Integer.parseInt(m.group(4)));
                    } catch (Exception e) {
                        e.printStackTrace();
                        int[] ports = stream.getDestinationPorts();
                        Log.d(TAG, "Server did not specify ports, using default ports: " + ports[0]
                                + "-" + ports[1]);
                    }
                } else {
    ...
    static class Response {
        // Parses method & uri
        public static final Pattern regexStatus = Pattern.compile("RTSP/\\d.\\d (\\d+) (\\w+)",
                Pattern.CASE_INSENSITIVE);
        // Parses a request header
        public static final Pattern rexegHeader = Pattern.compile("(\\S+):(.+)",
                Pattern.CASE_INSENSITIVE);
        // Parses a WWW-Authenticate header
        public static final Pattern rexegAuthenticate = Pattern.compile(
