...
public class RtspClient {
    public static final String TAG = RtspClient.class.getSimpleName();
    ...
    public static final int ERROR_WRONG_CREDENTIALS = 0x03;

    /** Use this to use UDP for the transport protocol. */
    public static final int TRANSPORT_UDP = RtpSocket.TRANSPORT_UDP;

    /** Use this to use TCP for the transport protocol. */
    public static final int TRANSPORT_TCP = RtpSocket.TRANSPORT_TCP;
     ...
     **/
    public static final int ERROR_CONNECTION_LOST = 0x04;
     ...
     * reconnect as long as {@link #stopStream()} is not called.
     */
    public static final int MESSAGE_CONNECTION_RECOVERED = 0x05;

    private static final int STATE_STARTED = 0x00;
    private static final int STATE_STARTING = 0x01;
    private static final int STATE_STOPPING = 0x02;
    private static final int STATE_STOPPED = 0x03;
    ...
    private final HandlerThread mHandlerThread = new HandlerThread("RtspClient");
    ...
    /**
     * If authentication is enabled on the server, you need to call this with a valid login/password
     * pair.
     ...
     * You need to call {@link #setServerAddress(String, int)} and optionally
     * {@link #setSession(Session)} and {@link #setCredentials(String, String)} before calling this.
    ...
    public void startStream() {
        ...
        mHandler.post(() -> {
            ...
            // If the user calls some methods to configure the client, it won't modify its
            // behavior until the stream is restarted
    ...
    private void sendRequestAnnounce() throws IllegalStateException, SocketException, IOException {
        ...
        String request =
                "ANNOUNCE rtsp://" + mParameters.host + ":" + mParameters.port + mParameters.path
                        + " RTSP/1.0\r\n"
                        + "CSeq: " + (++mCSeq) + "\r\n"
        ...
        if (response.headers.containsKey("session")) {
            try {
                Matcher m = Response.REGEX_SESSION.matcher(response.headers.get("session"));
                m.find();
        ...
        if (response.status == 401) {
            ...
            if (mParameters.username == null || mParameters.password == null) {
                throw new IllegalStateException(
                        "Authentication is enabled and setCredentials() was not called!");
            }

            try {
                m = Response.REGEX_AUTHENTICATE.matcher(response.headers.get("www-authenticate"));
                m.find();
                nonce = m.group(2);
                realm = m.group(1);
            } catch (Exception e) {
            ...
            String hash1 = computeMd5Hash(
                    mParameters.username + ":" + m.group(1) + ":" + mParameters.password);
            String hash2 = computeMd5Hash("ANNOUNCE" + ":" + uri);
            String hash3 = computeMd5Hash(hash1 + ":" + m.group(2) + ":" + hash2);

            mAuthorization = "Digest username=\"" + mParameters.username + "\",realm=\"" + realm
                    + "\",nonce=\"" + nonce + "\",uri=\"" + uri + "\",response=\"" + hash3 + "\"";

            request = "ANNOUNCE rtsp://" + mParameters.host + ":" + mParameters.port
                    + mParameters.path + " RTSP/1.0\r\n"
                    + "CSeq: " + (++mCSeq) + "\r\n"
                    + "Content-Length: " + body.length() + "\r\n"
        ...
        for (int i = 0; i < 2; i++) {
            ...
            if (stream != null) {
                String params = mParameters.transport == TRANSPORT_TCP
                        ...
                        : ("UDP;unicast;client_port=" + (5000 + 2 * i) + "-" + (5000 + 2 * i + 1)
                                + ";mode=receive");
                String request = "SETUP rtsp://" + mParameters.host + ":" + mParameters.port
                        + mParameters.path + "/trackID=" + i + " RTSP/1.0\r\n"
                ...
                if (response.headers.containsKey("session")) {
                    try {
                        m = Response.REGEX_SESSION.matcher(response.headers.get("session"));
                        m.find();
                        mSessionID = m.group(1);
                    } catch (Exception e) {
                        throw new IOException(
                                "Invalid response from server. Session id: " + mSessionID);
                    }
                }

                if (mParameters.transport == TRANSPORT_UDP) {
                    try {
                        m = Response.REGEX_TRANSPORT.matcher(response.headers.get("transport"));
                        m.find();
                        stream.setDestinationPorts(Integer.parseInt(m.group(3)),
                                Integer.parseInt(m.group(4)));
                        Log.d(TAG, "Setting destination ports: " + Integer.parseInt(m.group(3))
                                + ", " + Integer.parseInt(m.group(4)));
                    } catch (Exception e) {
                        e.printStackTrace();
                        int[] ports = stream.getDestinationPorts();
                        Log.d(TAG, "Server did not specify ports, using default ports: "
                                + ports[0] + "-" + ports[1]);
                    }
                } else {
    ...
    private void sendRequestRecord() throws IllegalStateException, SocketException, IOException {
        String request =
                "RECORD rtsp://" + mParameters.host + ":" + mParameters.port + mParameters.path
                        + " RTSP/1.0\r\n"
                        + "Range: npt=0.000-\r\n"
                        + addHeaders();
        Log.i(TAG, request.substring(0, request.indexOf("\r\n")));
    ...
    private void sendRequestTeardown() throws IOException {
        String request =
                "TEARDOWN rtsp://" + mParameters.host + ":" + mParameters.port + mParameters.path
                        + " RTSP/1.0\r\n"
                        + addHeaders();
        Log.i(TAG, request.substring(0, request.indexOf("\r\n")));
        mOutputStream.write(request.getBytes(StandardCharsets.UTF_8));
    ...
    private void sendRequestOption() throws IOException {
        String request =
                "OPTIONS rtsp://" + mParameters.host + ":" + mParameters.port + mParameters.path
                        + " RTSP/1.0\r\n"
                        + addHeaders();
        Log.i(TAG, request.substring(0, request.indexOf("\r\n")));
        mOutputStream.write(request.getBytes(StandardCharsets.UTF_8));
    ...
    private String addHeaders() {
        return "CSeq: " + (++mCSeq) + "\r\n"
                + "Content-Length: 0\r\n"
                + "Session: " + mSessionID + "\r\n"
                // For some reason you may have to remove last "\r\n" in the next line to make
                // the RTSP client work with your wowza server :/
    ...
    static class Response {
        ...
        public static final Pattern REGEX_AUTHENTICATE = Pattern.compile(
                "realm=\"(.+)\",\\s+nonce=\"(\\w+)\"", Pattern.CASE_INSENSITIVE);
        // Parses a Session header
        public static final Pattern REGEX_SESSION = Pattern.compile("(\\d+)",
                Pattern.CASE_INSENSITIVE);
        // Parses a Transport header
        public static final Pattern REGEX_TRANSPORT = Pattern.compile(
                "client_port=(\\d+)-(\\d+).+server_port=(\\d+)-(\\d+)", Pattern.CASE_INSENSITIVE);
