...
public class VideoQuality {
     ...
     **/
    public static VideoQuality determineClosestSupportedResolution(Camera.Parameters parameters,
            VideoQuality quality) {
        VideoQuality v = quality.clone();
        int minDist = Integer.MAX_VALUE;
        StringBuilder supportedSizesSb = new StringBuilder("Supported resolutions: ");
        List<Size> supportedSizes = parameters.getSupportedPreviewSizes();
        for (Iterator<Size> it = supportedSizes.iterator(); it.hasNext(); ) {
            Size size = it.next();
            supportedSizesSb.append(size.width).append('x').append(size.height);
            if (it.hasNext()) {
                supportedSizesSb.append(", ");
            }
        ...
        Log.v(TAG, supportedSizesSb.toString());
        if (quality.resX != v.resX || quality.resY != v.resY) {
            Log.v(TAG, "Resolution modified: " + quality.resX + "x" + quality.resY + "->"
                    + v.resX + "x" + v.resY);
        }
    ...
    public static int[] determineMaximumSupportedFramerate(Camera.Parameters parameters) {
        ...
        StringBuilder supportedFpsRangesSb = new StringBuilder("Supported frame rates: ");
        List<int[]> supportedFpsRanges = parameters.getSupportedPreviewFpsRange();
        for (Iterator<int[]> it = supportedFpsRanges.iterator(); it.hasNext(); ) {
            int[] interval = it.next();
            // Intervals are returned as integers, for example "29970" means "29.970" FPS.
            supportedFpsRangesSb.append(interval[0] / 1000).append('-')
                    .append(interval[1] / 1000).append("fps");
            if (it.hasNext()) {
                supportedFpsRangesSb.append(", ");
            }
            if (interval[1] > maxFps[1] || (interval[0] > maxFps[0] && interval[1] == maxFps[1])) {
                maxFps = interval;
            }
        }
        Log.v(TAG, supportedFpsRangesSb.toString());
        return maxFps;
    }
}
