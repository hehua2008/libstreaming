...
public class CodecManager {
    ...
    /**
     * There currently is no way to know if an encoder is software or hardware from the
    ...
    /**
     * Contains a list of encoders and color formats that we may use with a {@link
     * CodecManager.Translator}.
     */
    static class Codecs {
    ...
    static class Selector {
        private static final HashMap<String, SparseArray<ArrayList<String>>> sHardwareCodecs =
                new HashMap<>();
        private static final HashMap<String, SparseArray<ArrayList<String>>> sSoftwareCodecs =
                new HashMap<>();

        /**
         * Determines the most appropriate encoder to compress the video from the Camera
         */
        public static Codecs findCodecsFormMimeType(String mimeType,
                boolean tryColorFormatSurface) {
            ...
            // On devices running 4.3, we need an encoder supporting the color format used to
            // work with a Surface
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN_MR2
                ...
                if (list.hardwareCodec != null) {
                    Log.v(TAG,
                            "Choosen primary codec: " + list.hardwareCodec + " with color format: "
                                    + list.hardwareColorFormat);
                } else {
                    Log.e(TAG, "No supported hardware codec found !");
                }
                if (list.softwareCodec != null) {
                    Log.v(TAG, "Choosen secondary codec: " + list.hardwareCodec
                            + " with color format: " + list.hardwareColorFormat);
                } else {
            ...
            if (list.hardwareCodec != null) {
                Log.v(TAG, "Choosen primary codec: " + list.hardwareCodec + " with color format: "
                        + list.hardwareColorFormat);
            } else {
                Log.e(TAG, "No supported hardware codec found !");
            }
            if (list.softwareCodec != null) {
                Log.v(TAG, "Choosen secondary codec: " + list.hardwareCodec + " with color format: "
                        + list.softwareColorFormat);
            } else {
                Log.e(TAG, "No supported software codec found !");
            }
            return list;
        ...
        /**
         * Returns an associative array of the supported color formats and the names of the encoders
