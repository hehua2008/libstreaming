 ...
 * that encoders available through the MediaCodec API may have. <br />
...
public class EncoderDebugger {
    ...
    private void reset() {
        ...
        mErrorLog.delete(0, mErrorLog.length());
        mPPS = null;
        mSPS = null;
    }
    ...
    private void debug() {
        ...
        for (int i = 0; i < encoders.length; i++) {
            for (int j = 0; j < encoders[i].formats.length; j++) {
                ...
                try {
                    ...
                    saveTestResult(true);
                    Log.v(TAG, "The encoder " + mEncoderName + " is usable with resolution "
                            + mWidth + "x" + mHeight);
                    return;
                } catch (Exception e) {
        ...
        String errMsg = "No usable encoder were found on the phone for resolution "
                + mWidth + "x" + mHeight;
        Log.e(TAG, errMsg);
        ...
        if (mDecOutputFormat != null) {
            ...
            if (format != null) {
                ...
                if (format.containsKey(MediaFormat.KEY_COLOR_FORMAT)
                        && format.getInteger(MediaFormat.KEY_COLOR_FORMAT) > 0) {
