...
/**
 * The purpose of this class is to detect and by-pass some bugs (or underspecified configuration)
 * that
...
public class EncoderDebugger {
    public static final String TAG = EncoderDebugger.class.getSimpleName();
    ...
    public synchronized static void asyncDebug(final Context context, final int width,
            final int height) {
        new Thread(() -> {
            try {
    ...
    public synchronized static EncoderDebugger debug(SharedPreferences prefs, int width,
            int height) {
    ...
    private void debug() {
        ...
        if (!checkTestNeeded()) {
            ...
            if (!success) {
                throw new RuntimeException(
                        "Phone not supported with this resolution (" + mWidth + "x" + mHeight
                                + ")");
            }
            ...
            mNV21.setColorPanesReversed(
                    mPreferences.getBoolean(PREF_PREFIX + resolution + "reversed", false));
            ...
            for (int j = 0; j < encoders[i].formats.length; j++) {
                ...
                if (VERBOSE) {
                    Log.v(TAG, ">> Test " + (n++) + "/" + count + ": " + mEncoderName
                            + " with color format " + mEncoderColorFormat + " at " + mWidth + "x"
                            + mHeight);
                }
                ...
                try {
                    ...
                    for (int k = 0; k < decoders.length && !decoded; k++) {
                        for (int l = 0; l < decoders[k].formats.length && !decoded; l++) {
                            ...
                            try {
                                ...
                                if (VERBOSE) {
                                    Log.d(TAG, mDecoderName + " can't be used with "
                                            + mDecoderColorFormat + " at " + mWidth + "x"
                                            + mHeight);
                                }
                                releaseDecoder();
                                break;
                            ...
                            try {
                                decode(true);
                                if (VERBOSE) {
                                    Log.d(TAG, mDecoderName
                                            + " successfully decoded the NALs (color format "
                                            + mDecoderColorFormat + ")");
                                }
                                decoded = true;
                            } catch (Exception e) {
                    ...
                    saveTestResult(true);
                    Log.v(TAG,
                            "The encoder " + mEncoderName + " is usable with resolution " + mWidth
                                    + "x" + mHeight);
                    return;
                } catch (Exception e) {
                    ...
                    String str = "Encoder " + mEncoderName + " cannot be used with color format "
                            + mEncoderColorFormat;
                    if (VERBOSE) {
                        Log.e(TAG, str, e);
                    }
        ...
        Log.e(TAG, "No usable encoder were found on the phone for resolution " + mWidth + "x"
                + mHeight);
        throw new RuntimeException(
                "No usable encoder were found on the phone for resolution " + mWidth + "x"
                        + mHeight);
    }
    ...
    private void convertToNV21(int k) {
        ...
        if (mDecOutputFormat != null) {
            ...
            if (format != null) {
                ...
                if (format.containsKey(MediaFormat.KEY_COLOR_FORMAT) && format.getInteger(
                        MediaFormat.KEY_COLOR_FORMAT) > 0) {
    ...
    private long searchSPSandPPS() {
        ...
        while (elapsed < 3000000 && (mSPS == null || mPPS == null)) {
            ...
            // inconsistent, I have observed that some
            // encoders will give those parameters through the MediaFormat object (that is the
            // normal behaviour).
            ...
            if (index == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED) {
                ...
                if (len < 128) {
                    ...
                    if (len > 0 && csd[0] == 0 && csd[1] == 0 && csd[2] == 0 && csd[3] == 1) {
                        // Parses the SPS and PPS, they could be in two different packets and in
                        // a different order
                        //depending on the phone so we don't make any assumption about that
                        while (p < len) {
                            while (!(csd[p + 0] == 0 && csd[p + 1] == 0 && csd[p + 2] == 0
                                    && csd[p + 3] == 1) && p + 3 < len) {
                                p++;
                            }
                            if (p + 3 >= len) {
    ...
    /**
     * @param withPrefix If set to true, the decoder will be fed with NALs preceeded with
     *                   0x00000001.
        ...
        while (elapsed < 3000000) {
            ...
            if (i < NB_ENCODED) {
                ...
                if (decInputIndex >= 0) {
                    ...
                    decInputBuffers[decInputIndex].clear();
                    if ((withPrefix && hasPrefix(mVideo[i])) || (!withPrefix && !hasPrefix(
                            mVideo[i]))) {
