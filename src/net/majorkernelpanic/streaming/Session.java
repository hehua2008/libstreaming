 ...
 *
 * It holds a {@link BaseVideoStream} and a {@link AudioStream} together and provides
 * synchronous and asynchronous functions to start and stop those steams.
 * You should implement a callback interface {@link Callback} to receive notifications and error
 * reports.<br />
 *
 * If you want to stream to a RTSP server, you will need an instance of this class and hand it to a
 * {@link RtspClient}.
 *
 * If you don't use the RTSP protocol, you will still need to send a session description to the
 * receiver for him to be able to decode your audio/video streams. You can obtain this session
 * description by calling {@link #configure()} or {@link #syncConfigure()} to configure the
 * session with its parameters (audio samplingrate, video resolution) and then
...
public class Session {
    ...
    public static final int STREAM_VIDEO = 0x01;
    public static final int STREAM_AUDIO = 0x00;

    /** Some app is already using a camera (Camera.open() has failed). */
    public static final int ERROR_CAMERA_ALREADY_IN_USE = 0x00;

    /**
     * The phone may not support some streaming parameters that you are trying to use (bit rate,
     * frame rate, resolution...).
     */
    public static final int ERROR_CONFIGURATION_NOT_SUPPORTED = 0x01;
     ...
     */
    public static final int ERROR_STORAGE_NOT_READY = 0x02;

    /** The phone has no flash. */
    public static final int ERROR_CAMERA_HAS_NO_FLASH = 0x03;

    /** The supplied SurfaceView is not a valid surface, or has not been created yet. */
    public static final int ERROR_INVALID_SURFACE = 0x04;
     ...
     */
    public static final int ERROR_UNKNOWN_HOST = 0x05;

    /**
     * Some other error occurred !
     */
    public static final int ERROR_OTHER = 0x06;
    ...
    private BaseVideoStream mBaseVideoStream = null;

    private Callback mCallback;

    private final HandlerThread mHandlerThread = new HandlerThread("Session");
    ...
    public Session() {
        ...
        mTimestamp = (uptime / 1000) << 32
                & (((uptime - ((uptime / 1000) * 1000)) >> 32) / 1000); // NTP timestamp
        mOrigin = "127.0.0.1";
    }
    ...
    public interface Callback {
        ...
        /**
         * Called when the previw of the {@link BaseVideoStream}
    ...
    /** You probably don't need to use that directly, use the {@link SessionBuilder}. */
    void addVideoTrack(BaseVideoStream track) {
        removeVideoTrack();
        mBaseVideoStream = track;
    }
    ...
    void removeVideoTrack() {
        if (mBaseVideoStream instanceof VideoStream) {
            ((VideoStream) mBaseVideoStream).stopPreview();
            mBaseVideoStream = null;
        }
    }
    ...
    /** Returns the underlying {@link BaseVideoStream} used by the {@link Session}. */
    public BaseVideoStream getVideoTrack() {
        return mBaseVideoStream;
    }
    ...
    public void setVideoQuality(VideoQuality quality) {
        if (mBaseVideoStream != null) {
            mBaseVideoStream.setVideoQuality(quality);
        }
    }
    ...
    public void setSurfaceView(final SurfaceView view) {
        mHandler.post(() -> {
            if (mBaseVideoStream instanceof VideoStream) {
                ((VideoStream) mBaseVideoStream).setSurfaceView(view);
            }
        });
    }
    ...
    public void setPreviewOrientation(int orientation) {
        if (mBaseVideoStream instanceof VideoStream) {
            ((VideoStream) mBaseVideoStream).setPreviewOrientation(orientation);
        }
    }
     ...
     * @throws IllegalStateException Thrown when {@link #setDestination(String)} has never been
     *                               called.
     */
    public String getSessionDescription() {
        ...
        sessionDescription.append("o=- ").append(mTimestamp).append(' ').append(mTimestamp)
                .append(" IN IP4 ").append(mOrigin).append("\r\n");
        ...
        if (mAudioStream != null) {
            ...
            sessionDescription.append("a=control:trackID=").append(0).append("\r\n");
        }
        if (mBaseVideoStream != null) {
            sessionDescription.append(mBaseVideoStream.getSessionDescription());
            sessionDescription.append("a=control:trackID=").append(1).append("\r\n");
        }
    ...
    public long getBitrate() {
        ...
        if (mBaseVideoStream != null) {
            sum += mBaseVideoStream.getBitrate();
        }
        return sum;
    }
    ...
    public boolean isStreaming() {
        return (mAudioStream != null && mAudioStream.isStreaming())
                || (mBaseVideoStream != null && mBaseVideoStream.isStreaming());
    }
    ...
    public void syncConfigure()
        ...
        for (int id = 0; id < 2; id++) {
            Stream stream = id == 0 ? mAudioStream : mBaseVideoStream;
     ...
     * before calling this method. Otherwise, the {@link Callback#onSessionError(int, int,
     * Exception)}
    ...
    public void startPreview() {
        mHandler.post(() -> {
            if (mBaseVideoStream instanceof VideoStream) {
                try {
                    ((VideoStream) mBaseVideoStream).startPreview();
                    postPreviewStarted();
                    mBaseVideoStream.configure();
                } catch (CameraInUseException e) {
    ...
    public void switchCamera() {
        mHandler.post(() -> {
            if (mBaseVideoStream instanceof VideoStream) {
                try {
                    ((VideoStream) mBaseVideoStream).switchCamera();
                    postPreviewStarted();
                } catch (CameraInUseException e) {
                    postError(ERROR_CAMERA_ALREADY_IN_USE, STREAM_VIDEO, e);
                } catch (ConfNotSupportedException e) {
    ...
    public int getCamera() {
        return mBaseVideoStream instanceof VideoStream ?
                ((VideoStream) mBaseVideoStream).getCamera() : 0;
    }

    /**
     * Toggles the LED of the phone if it has one.
     * You can get the current state of the flash with
     * {@link Session#getVideoTrack()} and {@link BaseVideoStream#getFlashState()}.
     **/
    public void toggleFlash() {
        mHandler.post(() -> {
            if (mBaseVideoStream instanceof VideoStream) {
                try {
                    ((VideoStream) mBaseVideoStream).toggleFlash();
